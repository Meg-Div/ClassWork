...
app.get("/ages", (req, res) => {
  ...
  for (let i = 0; i < people.length; i++) {
    if (Number(people[i].age) === 24) {
      //slice + replace if needed
      obj.phoneNumber = people[i].phone;
      phones.push(obj);
    }
  }
  console.log(phones);
...
app.get("/users", (req, res) => {
  ...
  for (let i = 0; i < people.length; i++) {
    if (Number(people[i].age) > 21 && Number(people[i].age) <= 26) {
      users.push(people[i]);
    }
  }
  console.log(users);
...
//expected output[{email: "blah@blah.com"}]
...
[
  ...
  nameOfCompanies: [
    {username: "Annmarie", companyName: "SONGLINES",_id: "63bdb727c9bfbd1f62390e9b"},
  ],
  employeeAddresses: [
    {employee: "Annmarie", address: "659 Minna Street, Buxton, Northern Mariana Islands, 5397",_id: "63bdb727c9bfbd1f62390e9b"},
  ]
]
...
app.get("/compileYearEndReport", (request, response) => {
  let totalBalances;

  for (let i = 0; i < people.length; i++) {
    let num = people[i].balance;
    num = num.slice(1);
    num = parseFloat(num.replace(/,/g, "")).toFixed(2);
    console.log(num);
    totalBalances = totalBalances + num;
    console.log(totalBalances);
    console.log(num);
  }

  response.send(totalBalances);
});
